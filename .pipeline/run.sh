...
# Wait for backgrounded child, checking interrupt flag every second
wait_for_child() {
  while kill -0 "$CHILD_PID" 2>/dev/null; do
    if [ "$INTERRUPTED" -eq 1 ]; then
      echo ""
      echo "⏳ Just a minute, shutting down gracefully..."
    ...
    # Background sleep so SIGINT goes to us, not to sleep
    sleep 0.5 &
...
run_claude_piped() {
  ...
  local exit_code=$?
  CHILD_PID=""
  [ "$INTERRUPTED" -eq 1 ] && { echo ""; echo "⏳ Just a minute, shutting down gracefully..."; exit 130; }
  return $exit_code
}
...
run_step() {
  ...
  # Check if interrupted during step execution
  [ "$INTERRUPTED" -eq 1 ] && { log "Interrupted after $step_name"; exit 130; }
...
# Skip usage check at start (too slow, opens interactive Claude in tmux)
...
while [ "$phase" -le "$MAX_PHASES" ]; do
  ...
  while [ "$current_step" != "done" ]; do
    [ "$INTERRUPTED" -eq 1 ] && { log "Interrupted between steps"; exit 130; }
